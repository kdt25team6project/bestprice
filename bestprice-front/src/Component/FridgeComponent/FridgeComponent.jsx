...
function FridgeComponent() {
    ...
    return (
        <div className="fridge-container"> {/* 냉장고 컴포넌트의 메인 컨테이너 */}
            {/* 왼쪽 패널을 여는 버튼 - 식료품 추가하기 */}
            <button className="add-button" onClick={toggleLeftCanvas}>추가하기</button>
            <div className="fridge"> {/* 냉장고를 나타내는 컨테이너 */}
                <div className="fridge-body"> {/* 냉장고의 본체 */}
                    {/* 상단 냉장고 문 - 클릭하여 열거나 닫기 */}
                    <div className={`fridge-door fridge-door-top ${isTopDoorOpen ? 'door-open' : ''}`} onClick={toggleTopDoor}>
                        {/* 문이 닫혀 있을 때 손잡이를 표시 */}
                        {!isTopDoorOpen && <div className="door-handle door-handle-top"></div>}
                        {/* 문이 열렸을 때 내부 문을 표시 */}
                        {isTopDoorOpen && (<div className="fridge-compartment-door fridge-compartment-door-top"></div>)}
                    </div>
                    {/* 상단 냉장고 칸 - 문이 열렸을 때 보임 */}
                    <div className={`fridge-compartment fridge-compartment-top ${isTopDoorOpen ? 'visible' : ''}`}></div>
                    {/* 하단 냉장고 문 - 클릭하여 열거나 닫기 */}
                    <div className={`fridge-door fridge-door-bottom ${isBottomDoorOpen ? 'door-open' : ''}`} onClick={toggleBottomDoor}>
                        {/* 문이 닫혀 있을 때 손잡이를 표시 */}
                        {!isBottomDoorOpen && <div className="door-handle door-handle-bottom"></div>}
                        {/* 문이 열렸을 때 내부 문을 표시 */}
                        {isBottomDoorOpen && (<div className="fridge-compartment-door fridge-compartment-door-bottom"></div>)}
                    </div>
                    {/* 하단 냉장고 칸 - 문이 열렸을 때 보임 */}
                    <div className={`fridge-compartment fridge-compartment-bottom ${isBottomDoorOpen ? 'visible' : ''}`}></div>
                </div>
            </div>
            {/* 오른쪽 패널을 여는 버튼 - 더보기 */}
            ...
            <div className={`off-canvas left-canvas ${isLeftCanvasOpen ? 'open' : ''}`}>
                ...
                <div className="left-canvas-content"> {/* 왼쪽 패널 내용 */}
                    <h2>식료품 입력하기</h2>
                    {/* 카테고리 선택 버튼 */}
                    <div className="category-buttons">
                        {['채소', '과일', '고기', '수산물', '유제품', '음료', '주류', '잡곡'].map((category) => (
                            <button
                                key={category}
                    ...
                    <div className="selected-category"> {/* 선택된 카테고리 */}
                        {/* Emoji 선택기 추가 */}
            ...
            <div className={`off-canvas right-canvas ${isRightCanvasOpen ? 'open' : ''}`}>
                ...
                <div className="right-canvas-content"> {/* 오른쪽 패널 내용 */}
                    <h2>유통기한 알림</h2>
                    {[1, 2, 3, 4, 5].map((item, index) => (
                        <div key={index} className="expiration-item"> {/* 유통기한이 다가오는 항목 */}
                            <div className="icon-placeholder">아이콘</div> {/* 아이콘 자리 */}
                            <div className="expiration-details"> {/* 유통기한 정보 */}
                    ...
                    <div className="recipe-links">
                        {[1, 2, 3, 4, 5].map((item, index) => (
                            <div key={index} className="recipe-item"> {/* 레시피 항목 */}
